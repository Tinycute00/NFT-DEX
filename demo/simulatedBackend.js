...
var FakeContract = {
  ...
  buyNFT: function(tokenId) {
    ...
    // Record the actual transaction price including the 3% fee
    nft.lastTransactionPrice = price + (price * 0.03);
    // 更新 NFT 的底價為最新交易價格
    nft.basePrice = nft.lastTransactionPrice;
    // If the NFT is listed in the user market, credit the seller
    if(listing.listingType === "user") {
      var seller = listing.seller;
      this.users[seller].eth += price;
    }
    ...
    // 將手續費 3% 累加到全局溢價池
    var fee = price * 0.03;
    window.premiumPool = (window.premiumPool || 0) + fee;
  ...
  sellNFT: function(tokenId) {
    ...
    // 將全局溢價池取出
    var premiumPool = window.premiumPool || 0;
    // 計算該 NFT 分得的溢價：根據 NFT 的稀有度比例分配
    var allocatedPremium = (nft.rarity / this.totalRaritySum) * premiumPool;
    // 系統報價 = 當前底價 + 分配到的溢價
    var price = nft.basePrice + allocatedPremium;
    // 將 NFT 轉給系統
    nft.owner = "SYSTEM";
    this.listings[tokenId] = { price: price, seller: this.currentUser };
    // 賣出後立即將售價金額記入賣方餘額
    this.users[this.currentUser].eth += price;
    // 從全局溢價池中扣除該 NFT 分得的溢價
    window.premiumPool = premiumPool - allocatedPremium;
    // 更新 NFT 的底價為此次交易報價，反映最新成交
    nft.basePrice = price;
    this.updateWalletDisplay();
  ...
  /* 新增: 用戶市場掛牌出售 NFT 的函數 */
  listNFTForUserMarket: function(tokenId, price) {
    var nft = this.NFTs[tokenId];
    if (!nft) {
      alert("NFT 不存在");
      return null;
    ...
    if (nft.owner !== this.currentUser) {
      alert("你不是 NFT 的擁有者，無法掛牌");
      return null;
    }
    // 在 listings 中記錄用戶市場掛牌，標記 listingType 為 "user"
    this.listings[tokenId] = { price: price, seller: this.currentUser, listingType: "user" };
    alert("NFT 已掛牌出售，價格：" + price + " ETH");
    return { tokenId: tokenId, listingPrice: price, listingType: "user" };
  },
};
